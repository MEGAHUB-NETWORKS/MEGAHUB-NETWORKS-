...
window.MegaHub = {
  state: {
    nickname: localStorage.getItem('mh_nick') || 'Gamer_' + Math.floor(Math.random() * 9999),
  ...
  notify(txt) {
    ...
    n.className = 'glass';
    n.style.cssText = 'position:fixed; bottom:100px; left:50%; transform:translateX(-50%); padding:12px 24px; z-index:9999; border-left:4px solid #3b82f6; pointer-events:none; font-weight:900; animation: slide-up-fade 0.5s forwards; letter-spacing: 2px; text-transform: uppercase; font-size: 0.8rem;';
    n.textContent = txt;
  ...
  checkDaily() {
    ...
    if (last !== today) {
      ...
      this.addCoins(200);
      this.notify("Daily Login Reward: 200 Coins!");
    }
  },
...
window.Shop = {
  handleAction(id) {
    ...
    if (window.MegaHub.state.inventory.includes(id)) {
      ...
      window.MegaHub.notify("EQUIP STATUS UPDATED");
    } else {
      if (window.MegaHub.state.coins >= item.price) {
        ...
        localStorage.setItem('mh_eq', JSON.stringify(window.MegaHub.state.equipped));
        window.MegaHub.notify("ITEM ACQUIRED");
      } else {
        window.MegaHub.notify("INSUFFICIENT CREDITS");
      }
    }
  }
};

window.onload = () => MegaHub.init();
